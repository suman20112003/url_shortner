...
async function createShortUrl(req, res) {
    ...
    try {
        ...
        if(urlDoc) {
            // Regenerate a new short id for an existing URL so the short link changes on repeat requests.
            const shortUrlId = await generateUniqueId()
            urlDoc.shortUrlId = shortUrlId
            urlDoc.clicks = 0
            urlDoc.date = new Date()
            await urlDoc.save()

            const shortUrl = `${clientUrl}/${shortUrlId}`
            res.status(200).json({shortUrl: shortUrl, clicks: 0})
            console.log('Url already present. Generated new short url', shortUrl)
            return
        }
